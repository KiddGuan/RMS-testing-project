...
class GPINFO(object):
    ...
    def get_data(self):
        ...
        for a in soup.find('div',class_='quotebody').find_all('a',{'target':'_blank'}):
            record_d = {}
            #代号
            num = a.get_text().split('(')[1].strip(')')  #获取股票代号 outstanding 循环结束处理
    ...
    #将梳理数据写入到记录文件(json格式存储)
def printoutput(Outputf,Outrec):
    with open(Outputf,'ab') as f:
        f.write((Outrec+'\n').encode('utf-8'))
            #f.write(i)
    #f.close(Outrecf)
...
def main():
    test = GPINFO()
    result = test.BaseData  ##所获取的股票清单
    Date = time.strftime('%Y%m%d')
    Outputf = 'Outputlist'+Date  
    Outrec=[]
    #[浦发银行，总市值    净资产    净利润    市盈率    市净率    毛利率    净利率    ROE] roe:净资产收益率]
    #print(result)
    print ('名称      总市值    净资产    净利润    市盈率    市净率    毛利率    净利率    ROE')
    #hdr= '[名称      总市值    净资产    净利润    市盈率    市净率    毛利率    净利率    ROE]'
    #if os.path.exists(Outputf):
    #   print ('Output file exist...')
    #else:
    #   printoutput(Outputf,hdr)
            
    #top_10 = heapq.nlargest(10,result,key=lambda r:float(r['data'][7].strip('%')))   #获取前10名利率最高者的数据
    top_10 = heapq.nlargest(10,result,key=lambda r:float(r['data'][7].strip('%')))   #获取前10名利率最高者的数据
    for item in top_10:
        print (item['num'])
        Outrec.append(item['num'])
        for key in item['data']:
            print(key)
            Outrec.append(key)
        print(Outrec)
        Outrec.append('\n')
        #printoutput(Outputf,Outrec)
        Outrec=[]
            #txt=key
            #printoutput(Outputf,txt)
        print('\n')
